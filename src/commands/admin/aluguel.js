...
module.exports = {
  ...
  usage: `${PREFIX}aluguel [adicionar|consultar|renovar|remover|listar]\n\n` +
         `Exemplos:\n` +
         `${PREFIX}aluguel adicionar @usuario 31/12/2025\n` +
      ...
      case "add": {
        ...
        if (!mentionedJidList || mentionedJidList.length === 0) {
          throw new InvalidParameterError(
            ...
            `Exemplo: ${PREFIX}aluguel adicionar @usuario 31/12/2025`
          );
        }

        // Encontra a data nos argumentos (ignora a menção)
        let dataRenovacao = null;
        for (let i = 1; i < args.length; i++) {
          if (args[i].includes("/") && validateDate(args[i])) {
            dataRenovacao = args[i];
            break;
          }
        }
        ...
        const usuarioJid = mentionedJidList[0];
        
        // Extrai o número do JID (funciona com lid e número normal)
        let usuarioNumero = usuarioJid.split("@")[0];
        
        // Se for lid, tenta extrair o número (formato: 0:NUMERO ou lid:NUMERO)
        if (usuarioNumero.includes(":")) {
          usuarioNumero = usuarioNumero.split(":")[1] || usuarioNumero;
        }
        
        // Remove caracteres não numéricos, mantendo apenas o número
        usuarioNumero = usuarioNumero.replace(/\D/g, "");
      ...
      case "info": {
        ...
        if (!aluguel) {
          throw new WarningError(
            ...
            `Use: ${PREFIX}aluguel adicionar @usuario 31/12/2025`
          );
        }
