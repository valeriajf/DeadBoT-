...
async function findRealPhoneNumber(socket, remoteJid, userJid, webMessage) {
  try {
    console.log(`🔍 Buscando número real para JID: ${userJid}`);
    
    // 1. Se já é um JID normal (@s.whatsapp.net), retorna direto
    if (userJid.includes('@s.whatsapp.net')) {
      const number = userJid.split('@')[0];
      console.log(`✅ Número extraído do JID: ${number}`);
      return number;
    }
    
    // 2. Tenta extrair da mensagem original (webMessage)
    if (webMessage) {
      // Verifica se a mensagem respondida tem o pushName ou sender info
      const quotedMsg = webMessage.message?.extendedTextMessage?.contextInfo?.quotedMessage;
      const quotedStanza = webMessage.message?.extendedTextMessage?.contextInfo?.stanzaId;
      
      if (quotedStanza) {
        console.log(`📨 Mensagem respondida detectada: ${quotedStanza}`);
      }
    }
    
    // 3. Busca nos metadados do grupo
    try {
      const metadata = await socket.groupMetadata(remoteJid);
      console.log(`👥 Total de participantes: ${metadata.participants.length}`);
      
      // Procura nos participantes por JID alternativo
      for (const p of metadata.participants) {
        // Se encontrar correspondência por LID
        if (p.id === userJid || p.lid === userJid) {
          console.log(`🎯 Participante encontrado: ${JSON.stringify(p)}`);
          
          // Verifica se tem JID normal
          if (p.id && p.id.includes('@s.whatsapp.net')) {
            const number = p.id.split('@')[0];
            console.log(`✅ Número obtido dos metadados (id): ${number}`);
            return number;
          }
        }
      }
    } catch (e) {
      console.log(`⚠️ Erro ao buscar metadados: ${e.message}`);
    }
    
    // 4. Tenta via store do socket
    if (socket.store && socket.store.contacts) {
      const contact = socket.store.contacts[userJid];
      if (contact) {
        console.log(`📇 Contato na store: ${JSON.stringify(contact)}`);
        
        if (contact.id && contact.id.includes('@s.whatsapp.net')) {
          const number = contact.id.split('@')[0];
          console.log(`✅ Número obtido da store: ${number}`);
          return number;
        }
      }
    }
    
    // 5. Último recurso: retorna o LID
    const lidNumber = userJid.split('@')[0];
    console.log(`⚠️ Não conseguiu converter LID. Retornando: ${lidNumber}`);
    return lidNumber;
...
async function addToBlacklist(socket, remoteJid, userJid, webMessage) {
  ...
  console.log(`📝 Adicionando à blacklist:`);
  console.log(`   JID original: ${userJid}`);
  console.log(`   Número encontrado: ${phoneNumber}`);
  console.log(`   É apenas LID? ${isLidOnly}`);
  ...
  if (!isLidOnly && phoneNumber.length >= 10) {
    ...
    console.log(`✅ Salvo com número: ${phoneNumber}`);
  } else {
    console.log(`⚠️ Salvo apenas com LID (número não disponível)`);
  }
