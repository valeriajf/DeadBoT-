...
function isEventProcessed(remoteJid, userJid, action) {
  ...
  if (eventCache.has(key)) {
    const timestamp = eventCache.get(key);
  ...
  for (const [k, time] of eventCache.entries()) {
      ...
      if (action === "remove" || action === "leave") {
        if (fs.existsSync(EXIT2_PATH)) {
          ...
          if (groupExit && groupExit.active) {
            const exitUserNumber = userJid.split("@")[0];
            ...
            message = message.replace(/{membro}/g, `@${exitUserNumber}`);
