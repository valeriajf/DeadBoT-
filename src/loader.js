...
const { autoInitSchedules } = require("./utils/scheduleAutoInit"); // ← NOVO: Importa auto-inicializador
...
exports.load = (socket) => {
  ...
  // ====================================
  // AUTO-INICIALIZAÇÃO DE AGENDAMENTOS
  // ====================================
  // Inicializa agendamentos de abertura/fechamento de grupos
  // após 3 segundos (garante que o bot está totalmente conectado)
  setTimeout(() => {
    try {
      autoInitSchedules(socket);
    } catch (error) {
      errorLog(`Erro ao inicializar agendamentos: ${error.message}`);
    }
  }, 3000);
